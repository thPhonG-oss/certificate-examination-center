...
async function renderLichThi(list_lichthi) {
    ...
    try {
        ...
        var htmls = list_lichthi.map(function(lichthi, index) {
            ...
            const chungChi = chungChiList.find(c => String(c.id) === String(lichthi.certification_id));
            return `
...
async function addLichThi() {
    ...
    if (tenChungChiInput && ngayThiInput && gioThiInput && slToiDaInput) {
        ...
        const trungLich = lichThiList.filter(lt =>
            lt.certification_id === certification_id && lt.date === date
        );
        ...
        .then(function(lichThi) {
            if (lichThi) {
                ...
                fetchChungChi().then(function(chungChiList) {
                    // Tìm tên chứng chỉ theo certification_id
                    const chungChi = chungChiList.find(c => String(c.id) === String(lichThi.certification_id));
                    document.getElementById('modal_tenChungChi').value = chungChi ? chungChi.name : '';
                    document.getElementById('modal_ngayThi').value = lichThi.date;
                    document.getElementById('modal_gioThi').value = lichThi.time ? lichThi.time.slice(0,5) : '';
                    document.getElementById('modal_soLuongToiDa').value = lichThi.maxParticipants;
                    ...
                    btnSave.onclick = async function() {
                        ...
                        var maxParticipants = document.getElementById('modal_soLuongToiDa').value.trim();
                        ...
                        const chungChi2 = chungChiList2.find(c => c.name === ten_chung_chi);
                        if (!chungChi2) {
                            alert("Không tìm thấy chứng chỉ này!");
                            return;
                        }
