...
$(document).ready(() => {
  ...
  function setButtonsLoading(loading) {
    ...
    if (loading) {
      ...
      $submitBtn
        .prop("disabled", false)
        .html('<i class="fas fa-check-circle me-2"></i>Đăng ký');
  ...
  $('a[href="/api/auth/sign-out"], .logout').on("click", (e) => {
    ...
    if (isFormModified() && !isFormSubmitting) {
      showConfirmDialog(
        "Bạn có thay đổi chưa được lưu. Bạn có chắc chắn muốn đăng xuất?",
        () => {
          performLogout();
        }
      );
  ...
  $("#cancelRegistration").on("click", (e) => {
    ...
    if (isFormModified()) {
      showConfirmDialog(
        "Bạn có chắc chắn muốn hủy đăng ký? Tất cả thông tin đã nhập sẽ bị xóa.",
        () => {
          clearForm();
  ...
  function populateScheduleTable(schedules) {
    ...
    if (!schedules || schedules.length === 0) {
      $tbody.append(
        '<tr><td colspan="6" class="text-center">Không có lịch thi nào</td></tr>'
      );
      return;
    }
